...
Choose Focus or Break mode using the toggle at the top.
Press Start to begin the countdown.
Use Pause to stop the countdown temporarily, or Reset to restart the current mode from its full duration.
When a focus session completes, the counter increments automatically and the timer switches to break mode (and vice versa).
Project Structure
